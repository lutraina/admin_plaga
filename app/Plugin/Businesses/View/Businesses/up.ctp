		...
		<?php foreach($files as $img): ?>
			...
			<div class="span2" id="element<?= $img['File']['id']?>">
				...
				<div class="thumbnail">
					<div class="item">
							<div class="zoom">
								<img src="<?= $_URL_FILE; ?>businesses/fotos/640x480-<?= $img['File']['name'] ?>"/>
								<div class="zoom-icon"></div>
							</div>
						<div class="details">
							<a href="#?" class="icon" onclick="delteImage('<?= $img['File']['id']?>','element<?= $img['File']['id']?>')"><i class="icon-remove"></i></a>		
						</div>
					</div>
				</div>
				<div class="row-fluid">
...
<script>
	...
	$('.btn-cancelar-up').click(function(){
			...
			$.ajax({
					url:'/businesses/confirmar_arquivos/'+$("#businesses_id").val()+'/2',
					type:'POST',
					beforeSend: function(){
		...
		$('.btn-salvar-up').click(function(){
			...
			$.ajax({
					url:'/businesses/confirmar_arquivos/'+$("#businesses_id").val()+'/1',
					type:'POST',
					beforeSend: function(){
					...
					success: function(content) {
						...
						window.location.href="/businesses/view/"+content;
