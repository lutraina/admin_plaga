...
<div id="page-content" class="clearfix">
	<div class="page-header position-relative">
		<h1>
			<a href="/businesses">Estabelecimento Comercial</a>
			<small>
				<i class="icon-double-angle-right"></i>
				Gerencie os estabelecimentos
			</small>
			<span class="pull-right">
				<a href="<?= $_URL ?>businesses/add" class="btn btn-success btn-mini" >Adicionar um novo estabelecimento</a>
			</span>
		</h1>
	</div><!--/.page-header-->
...
<script src="/js/jquery.form.min.js"></script>
        <script>
				$(document).ready(function(){
				 ...
				 $('#upl').change(function(){
					 	if($('#businesses_id').val() != ''){
					 		
					 		uploadForm($('#businesses_id').val())
				            ...
				            success: function(data) {
				            	$('.preloader').hide('slow',function(){
				            		$('.upfotoshidden').show('fast',function(){
				            			...
				            			$('.upfoto').load('/businesses/up/'+galeria_id);
				            		})
	...
	function deleteGaleria(id){
		if(confirm("Deseja realmente excluir esta galeria?")){
			location.href="/businesses/delete/"+id;
		}
} 
