		...
		<?php foreach($files as $img): ?>
			...
			<div class="span2" id="element<?= $img['File']['id']?>">
				...
				<div class="thumbnail">
					<div class="item">
							<div class="zoom">
								<img src="<?= $_URL_FILE; ?>autos/fotos/B-<?= $img['File']['name'] ?>"/>
...
<script>
	...
	$('.btn-cancelar-up').click(function(){
			...
			$.ajax({
					url:'/autos/confirmar_arquivos/'+$("#autos_id").val()+'/2',
					type:'POST',
					beforeSend: function(){
		...
		$('.btn-salvar-up').click(function(){
			...
			$.ajax({
					url:'/autos/confirmar_arquivos/'+$("#autos_id").val()+'/1',
					type:'POST',
					beforeSend: function(){
					...
					success: function(content) {
						...
						window.location.href="/autos/view/"+content;
